...
app.addHook("onResponse", async (request, reply) => {
    ...
    app.log.info(`[RES] ${timestamp} | ${method} ${url} | ${statusCode} | ${responseTime}ms`);
});
async function main() {
    ...
    app.register(fastifyJWT, {
        secret: process.env.JWT_SECRET, // Use the JWT secret from your environment variable
    });
    ...
    if (process.env.NODE_ENV === "production" && enableApiDocs) {
        app.log.warn("⚠️ API documentation is enabled in production environment! This may expose sensitive endpoints. Consider setting ENABLE_API_DOCS=false for security.");
    }
