// src/routes/tournament/getTournamentById.ts
import { FastifyRequest, FastifyReply } from "fastify";
import { PrismaClient } from "@prisma/client";

const prisma = new PrismaClient();

interface GetTournamentByIdParams {
  id: string;
}
...
export async function getTournamentByIdHandler(
  request: FastifyRequest,
  reply: FastifyReply
) {
  try {
    const { id } = request.params as GetTournamentByIdParams;
    ...
    delete (responseTournament as any).tournamentParticipants;
